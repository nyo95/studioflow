...
| **R3** | Platform consolidation — audit fisik satu `AuditLog` generic, action errors, pagination, soft-delete | ✅ **selesai 2026-08-24** — migrasi `20260824150000_r3_audit_consolidation` mem-backfill & menghapus `MasterDataAudit`, semua writer audit lewat shared `recordAudit()`, reader Master Data pindah ke `AuditLog`, error wrapper menormalkan kode platform legacy, dan gerbang penuh lulus (`prisma validate`, typecheck, unit 209/209, integration docker 11/11, build ✓) |
| **R4** | Pricing reconciliation final: `SkuPrice` = current state per `(sku_id, supplier_party_id)`, `unit` mengikuti `purchase_unit`, updater identity/audit tersimpan, lifecycle history pindah ke `AuditLog` | ✅ **selesai 2026-08-24** — migrasi `20260824170000_r4_r9_r12_current_pricing_cleanup` menghapus `is_current`/`valid_from`/`valid_to`, mengarsipkan histori ke `studioflow.AuditLog`, menegakkan unique pair `(sku_id, supplier_party_id)`, dan memindahkan jalur tulis/read/impor Excel ke current-row update semantics |
...
| **R8** | Master Data ownership cleanup — `subapps/master-data` harus berhenti bergantung pada implementasi `extensions/library`; ownership katalog/vendor/SKU detail harus pindah ke boundary Master Data | ✅ **selesai 2026-08-24** — boundary type/read/write katalog kini hidup di `src/subapps/master-data/contracts/*`, `services/catalog-{read,write,sample}-service.ts`, dan `actions/catalog-{query,mutation}-actions.ts`; komponen Master Data tidak lagi mengimpor `@/extensions/library/*`, dan action Library yang setara kini mendelegasikan ke service Master Data yang sama |
| **R9** | BQ cleanup — explicit supplier-price selection, immutable acquisition snapshots, no refresh/drift semantics, deterministic ordering | ✅ **selesai 2026-08-24** — chooser material kini memilih `SKU + supplier price` secara eksplisit, snapshot menyimpan provenance supplier/price yang dipilih, `pickPrice()` dan field drift lama dihapus, line library recipe juga menyimpan `sku_price_id` / `supplier_party_id`; kalkulasi tetap server-only dan gate regresi lulus |
...
| **R11** | Boundary enforcement — lint/import guards, termasuk larangan reverse dependency `Master Data → extensions/library` | ✅ **selesai 2026-08-24** — `eslint.config.mjs` kini melarang `src/subapps/master-data/**` dan `src/app/masterdata/**` mengimpor `@/extensions/library*`; scan boundary terhadap `library-actions`, `types`, dan `library-service` nol, dan `npx eslint src` tetap bersih dari error |
| **R12** | Legacy purge — hanya setelah semua consumer termigrasi | 🟡 **parsial 2026-08-24** — `ProjectTimeline`, `TimelineTemplate`, `TimelineStatus`, util `project-timeline`, legacy pricing/drift fields, dan coupling aktif `Master Data → extensions/library` sudah dipurge; sisa pekerjaan kini normalization-only di sisi kompatibilitas/delegasi extension, bukan migrasi consumer hidup |
