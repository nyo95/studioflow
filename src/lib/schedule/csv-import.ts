...
async function resolveImportCategory(
  ...
  if (codePrefix) {
    ...
    if (prefixes.length > 1) {
      throw new ActionError(
        `Ambiguous category mapping for imported row ${row.code}: prefix "${codePrefix}" matches multiple categories [${prefixes.map(p => p.category).join(", ")}] in ${section.toLowerCase()} folder.`,
        "VALIDATION_FAILED"
      );
    }

    if (prefixes[0]?.category) {
      return prefixes[0].category;
    }
  }
...
export async function importScheduleFromCsv(
  ...
  for (const row of rows) {
    ...
    const category = normalizeImportCategory(existing?.category ?? await resolveImportCategory(tx, row, section));
    ...
    const entry = await tx.projectScheduleEntry.create({
      data: {
        ...
        section,
        code: `TEMP-IMPORT-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
