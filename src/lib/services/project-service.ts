...
export const projectService = {
  ...
  async executeDeleteProject(tx: PrismaTransaction, params: { projectId: string; userId: string }) {
    ...
    // 1. Masukkan log penghapusan TERLEBIH DAHULU (PENTING!)
    await insertAuditLog(tx, AUDIT_ACTIONS.DELETE_PROJECT, "Project", projectId, userId, { project_id: projectId });

    // 2. JANGAN hapus audit log (SSOT §7.3 & Audit Issue #2)
    // Audit log harus dipreservasi untuk akuntabilitas.

    // 3. Ubah ke Soft Delete (SSOT §7.1 & Audit Issue #3)
