...
/**
 * Terapkan nilai master data terkini ke satu baris — tombol "Review" pada
 * banner drift.
 *
 * SATU BARIS PER PANGGILAN. Itu yang membuat "boleh diterapkan sebagian"
 * (PRD §5.4) berlaku tanpa logika tambahan. Jangan menambahkan versi massal
 * "refresh semua" tanpa keputusan owner: sekali ada, ia akan jadi tombol yang
 * ditekan orang tanpa membaca, dan larangan silent update kehilangan artinya.
 */
export const refreshBqMaterialLineSnapshotAction = createAction(
  async ({ input, ctx, tx }) => {
    assertPerm(ctx.role, PERMISSION.BQ_BREAKDOWN_EDIT);

    const line = await tx.bqMaterialLine.findUnique({
      where: { id: input.id },
      select: { sub_object_id: true, sku_id: true },
    });
    if (!line) throw new ActionError("This line no longer exists.", "NOT_FOUND");
    if (!line.sku_id) {
      throw new ActionError(
        "This line is no longer linked to a Master Data material, so there is nothing to refresh.",
        "MASTER_GONE"
      );
    }
    await assertSubObjectEditable(tx, line.sub_object_id);
    ...
    const candidate = await loadMaterialCandidate(line.sku_id);
    if (!candidate || !candidate.readiness.ok) {
      throw new ActionError(
        candidate?.readiness.ok === false
          ? candidate.readiness.detail
          : "That material no longer exists in Master Data.",
        "NOT_FOUND"
      );
    }
    ...
    await tx.bqMaterialLine.update({
      ...
      data: {
        ...
        snapshot_price_valid_from: new Date(price.validFrom),
        snapshot_taken_at: new Date(),
        // Refresh dari master mengembalikan baris ini ke jalur normal.
    ...
    const projectId = await projectIdOfSubObject(tx, line.sub_object_id);
    await insertAuditLog(tx, "REFRESH", "BqMaterialLine", input.id, ctx.userId, {
      bq_project_id: projectId,
      sku_id: line.sku_id,
    });
...
export const refreshBqServiceLineSnapshotAction = createAction(
  async ({ input, ctx, tx }) => {
    assertPerm(ctx.role, PERMISSION.BQ_BREAKDOWN_EDIT);

    const line = await tx.bqServiceLine.findUnique({
      where: { id: input.id },
      select: { sub_object_id: true, work_price_id: true },
    });
    if (!line) throw new ActionError("This line no longer exists.", "NOT_FOUND");
    if (!line.work_price_id) {
      throw new ActionError(
        "This line is no longer linked to a Master Data service, so there is nothing to refresh.",
        "MASTER_GONE"
      );
    }
    await assertSubObjectEditable(tx, line.sub_object_id);
    ...
    const candidate = await loadServiceCandidate(line.work_price_id);
    if (!candidate) {
      throw new ActionError("That service no longer exists in Master Data.", "NOT_FOUND");
    }
    ...
    const projectId = await projectIdOfSubObject(tx, line.sub_object_id);
    await insertAuditLog(tx, "REFRESH", "BqServiceLine", input.id, ctx.userId, {
      bq_project_id: projectId,
