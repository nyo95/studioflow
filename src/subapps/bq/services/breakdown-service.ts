...
function toMaterialInput(row: MaterialRow): MaterialLineInput {
  return {
    ...
    usageUnit: row.snapshot_usage_unit,
    purchaseUnit: row.snapshot_purchase_unit,
    conversion: decToNumberStrict(row.snapshot_conversion),
...
export async function listProjects(): Promise<BqProjectSummary[]> {
  ...
  return rows.map((p) => ({
    ...
    name: p.name,
    status: p.status,
    studioflowProjectId: p.studioflow_project_id,
    studioflowProjectName: p.studioflow_project_name,
    notes: p.notes,
...
export async function loadProjectView(projectId: string): Promise<BqProjectView | null> {
  ...
  return {
    project: {
      ...
      studioflowProjectId: project.studioflow_project_id,
      studioflowProjectName: project.studioflow_project_name,
      notes: project.notes,
