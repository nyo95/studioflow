...
export function SkuDirectoryClient({
  ...
  React.useEffect(() => setSearch(initialFilters.search), [initialFilters.search]);
  ...
  const requestDeleteSku = async (row: MaterialRow) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "SKU could not be deleted");
    } finally {
  ...
  return (
    <DashboardPageShell>
      ...
      <div className="mb-[calc(var(--ui-section-gap)/2)] flex flex-wrap items-center justify-between gap-[calc(var(--ui-section-gap)/2)]">
        <p className={cn("text-[var(--ui-text-secondary)]", UI_ENGINE_TYPE_BODY)}>
      ...
      <TableCard layout="fixed" minWidth="var(--ui-sku-directory-table-min-width)">
        <TableCardHeader>
          <TableCardHead style={{ width: "var(--ui-sku-directory-col-brand)" }}>Brand</TableCardHead>
          <TableCardHead style={{ width: "var(--ui-sku-directory-col-product)" }}>SKU / Product</TableCardHead>
          <TableCardHead style={{ width: "var(--ui-sku-directory-col-category)" }}>Category</TableCardHead>
          <TableCardHead style={{ width: "var(--ui-sku-directory-col-unit)" }}>Base unit</TableCardHead>
          <TableCardHead align="right" style={{ width: "var(--ui-sku-directory-col-price)" }}>Best current price</TableCardHead>
          <TableCardHead align="center" style={{ width: "var(--ui-sku-directory-col-complete)" }}>Data</TableCardHead>
          <TableCardHead align="right" style={{ width: "var(--ui-sku-directory-col-actions)" }}>Actions</TableCardHead>
        </TableCardHeader>
        <TableCardBody>
          {rows.map((row) => (
            <TableCardRow
            ...
            >
              ...
              <TableCardCell>
                <div className="flex flex-col gap-[calc(var(--ui-section-gap)/5)]">
                  ...
                  <span className="text-[var(--ui-text-secondary)]">{row.productName}</span>
                </div>
              </TableCardCell>
              <TableCardCell className="text-[var(--ui-text-secondary)]">
                {row.categoryTags.length > 0 ? row.categoryTags.join(", ") : "No categories"}
              </TableCardCell>
              ...
              >
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onSelect={() => setViewerSkuId(row.id)}>
                      <Eye aria-hidden />
                      View details
                    </DropdownMenuItem>
                    {canManage ? (
      ...
      {rows.length === 0 ? (
        <p className={cn("p-[var(--ui-section-py)] text-center text-[var(--ui-text-tertiary)]", UI_ENGINE_TYPE_BODY)}>
      ...
      {total > 0 ? (
        <div className="mt-[calc(var(--ui-section-gap)/2)] flex flex-wrap items-center justify-between gap-[calc(var(--ui-section-gap)/2)]">
          <span className={cn("tabular-nums text-[var(--ui-text-secondary)]", UI_ENGINE_TYPE_META)}>
            Showing {rangeStart.toLocaleString("id-ID")}–{rangeEnd.toLocaleString("id-ID")} of {total.toLocaleString("id-ID")}
          </span>
          {totalPages > 1 ? (
