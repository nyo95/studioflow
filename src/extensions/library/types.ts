import { Prisma, Brand, PartyContact, BrandLink, LinkKind, PartyRoleKind, ProductType, PromotionRequest, Role, SkuStatus, SampleStatus } from "@/generated/prisma";
...
import type { SkuDto, SampleRowDto } from "./contracts/sku-dto";
import { z } from "zod";
 ...
 *
 * v2 has no `catalog_*` columns on Sku — pricing lives in `SkuPrice` (history),
 * images in `SkuMedia`, categories in `SkuCategory` -> `Category`. The
...
export type ProductCatalogInput = {
   ...
   * price rather than a shop's offer — the distinction `SkuPrice_current_uniq`
