...
export class LibraryService {
  ...
  static async getAllMaterials(
    ...
    if (filters?.search) {
      where.OR = [
        { product_type: { contains: filters.search, mode: "insensitive" } },
        { motif_or_color: { contains: filters.search, mode: "insensitive" } },
    ...
    return tx.materialCatalog.findMany({
      ...
      orderBy: [{ product_type: "asc" }, { created_at: "desc" }],
    });
  }
  ...
  static async createMaterial(data: MaterialCatalogInput, userId: string, tx: PrismaTransaction) {
    ...
    // Handle On-the-Go Vendor Creation
    if (!resolvedVendorId && data.vendor_name?.trim()) {
      const normalizedBrand = data.vendor_name.trim();
      const existingVendor = await tx.vendor.findFirst({
        where: { brand_name: { equals: normalizedBrand, mode: "insensitive" }, deleted_at: null }
      });
    ...
    if (!data.category?.trim()) throw new ActionError("Category is required.", "CATEGORY_REQUIRED");
    if (!data.product_type?.trim()) throw new ActionError("Product type is required.", "PRODUCT_TYPE_REQUIRED");
    ...
    const material = await tx.materialCatalog.create({
      data: {
        ...
        sub_category: this.normalizeOptional(data.sub_category),
        product_type: data.product_type.trim(),
        motif_or_color: this.normalizeOptional(data.motif_or_color),
        tags: data.tags || [],
        dimension_p: this.normalizeOptional(data.dimension_p),
        dimension_l: this.normalizeOptional(data.dimension_l),
        dimension_t: this.normalizeOptional(data.dimension_t),
        dimension_unit: data.dimension_unit || "cm",
        color: this.normalizeOptional(data.color),
        finishing: this.normalizeOptional(data.finishing),
        cover_url: this.normalizeOptional(data.cover_url),
        original_url: this.normalizeOptional(data.original_url),
        digital_catalog_url: this.normalizeOptional(data.digital_catalog_url),
        digital_folder_url: this.normalizeOptional(data.digital_folder_url),
        rak_location: this.normalizeOptional(data.rak_location),
        box_number: this.normalizeOptional(data.box_number),
  ...
  static async updateMaterial(id: string, data: Partial<MaterialCatalogInput>, userId: string, tx: PrismaTransaction) {
    if (data.category !== undefined && !data.category.trim()) throw new ActionError("Category is required.", "CATEGORY_REQUIRED");
    if (data.product_type !== undefined && !data.product_type.trim()) throw new ActionError("Product type is required.", "PRODUCT_TYPE_REQUIRED");
    ...
    const updated = await tx.materialCatalog.update({
      ...
      data: {
        ...
        ...(data.sub_category !== undefined ? { sub_category: this.normalizeOptional(data.sub_category) } : {}),
        ...(data.product_type ? { product_type: data.product_type.trim() } : {}),
        ...(data.motif_or_color !== undefined ? { motif_or_color: this.normalizeOptional(data.motif_or_color) } : {}),
        ...(data.tags !== undefined ? { tags: data.tags } : {}),
        ...(data.dimension_p !== undefined ? { dimension_p: this.normalizeOptional(data.dimension_p) } : {}),
        ...(data.dimension_l !== undefined ? { dimension_l: this.normalizeOptional(data.dimension_l) } : {}),
        ...(data.dimension_t !== undefined ? { dimension_t: this.normalizeOptional(data.dimension_t) } : {}),
        ...(data.dimension_unit !== undefined ? { dimension_unit: data.dimension_unit } : {}),
        ...(data.color !== undefined ? { color: this.normalizeOptional(data.color) } : {}),
        ...(data.finishing !== undefined ? { finishing: this.normalizeOptional(data.finishing) } : {}),
        ...(data.cover_url !== undefined ? { cover_url: this.normalizeOptional(data.cover_url) } : {}),
        ...(data.original_url !== undefined ? { original_url: this.normalizeOptional(data.original_url) } : {}),
        ...(data.digital_catalog_url !== undefined ? { digital_catalog_url: this.normalizeOptional(data.digital_catalog_url) } : {}),
        ...(data.digital_folder_url !== undefined ? { digital_folder_url: this.normalizeOptional(data.digital_folder_url) } : {}),
        ...(data.rak_location !== undefined ? { rak_location: this.normalizeOptional(data.rak_location) } : {}),
        ...(data.box_number !== undefined ? { box_number: this.normalizeOptional(data.box_number) } : {}),
  ...
  static async ensureMaterialInLibrary(tx: PrismaTransaction, data: {
    ...
    // Configurable via SCHEDULE_RESERVED_BRANDS env var
    const RESERVED_BRANDS = (process.env.SCHEDULE_RESERVED_BRANDS || "PENDING,RESERVED,[RESERVED]")
      .split(",")
      .map(b => b.trim().toUpperCase());
    
    if (RESERVED_BRANDS.includes(brand.toUpperCase())) {
    ...
    try {
      ...
      const existingMaterial = await tx.materialCatalog.findFirst({
        where: {
          vendor_id: vendor.id,
          product_type: { equals: name, mode: "insensitive" },
          deleted_at: null
        }
      });
      ...
      if (existingMaterial) {
        ...
        return tx.materialCatalog.update({
          ...
          data: {
            ...
            cover_url: data.image_url || existingMaterial.cover_url,
            price: data.price ?? existingMaterial.price,
            original_url: data.original_url || existingMaterial.original_url,
            color: data.color || existingMaterial.color,
            finishing: data.finishing || existingMaterial.finishing,
            motif_or_color: data.motif_or_color || existingMaterial.motif_or_color,
      ...
      return await tx.materialCatalog.create({
        data: {
          ...
          motif_or_color: data.motif_or_color,
          status: "PENDING"
        }
      });
    } catch (error: unknown) {
      ...
      if (error instanceof Error && error.message.includes("Unique constraint")) {
        ...
        if (foundMaterial) {
          ...
          return tx.materialCatalog.update({
            ...
            data: {
              category: data.category,
              cover_url: data.image_url || foundMaterial.cover_url,
              price: data.price ?? foundMaterial.price,
              original_url: data.original_url || foundMaterial.original_url,
              color: data.color || foundMaterial.color,
              finishing: data.finishing || foundMaterial.finishing,
              motif_or_color: data.motif_or_color || foundMaterial.motif_or_color,
            }
          });
        }
      }
  ...
  static async createProjectMaterialRequest(
    ...
    if (!materialId && data.custom_material_name) {
      ...
      const newMaterial = await tx.materialCatalog.create({
        data: {
          vendor_id: vendorId,
          category: "UNCATEGORIZED", // Default for harvested requests
          product_type: data.custom_material_name.trim(),
          status: "PENDING",
          cover_url: this.normalizeOptional(data.cover_url),
          original_url: this.normalizeOptional(data.original_url),
          digital_catalog_url: this.normalizeOptional(data.reference_url),
      ...
      await insertAuditLog(tx, AUDIT_ACTIONS.LIBRARY_CREATE_MATERIAL, "MaterialCatalog", materialId, userId, {
        product_type: newMaterial.product_type,
