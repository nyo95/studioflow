...
export function PhysicalInventoryTable({ materials, onEdit }: PhysicalInventoryTableProps) {
  ...
  const flattenedSamples = React.useMemo(() => {
    return materials.map(m => {
      ...
      if (m.rak_location || m.box_number) {
        return { 
          id: `flat-${m.id}`, 
          location_rak: m.rak_location || "-", 
          container_box: m.box_number || "-", 
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
            <TableHead className="font-sans font-semibold text-slate-900">Rak / Box</TableHead>
            <TableHead className="font-sans font-semibold text-slate-900">Material Info</TableHead>
            <TableHead className="font-sans font-semibold text-slate-900">Vendor</TableHead>
            <TableHead className="font-sans font-semibold text-slate-900">Notes</TableHead>
            <TableHead className="font-sans font-semibold text-slate-900 text-right">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
            ...
            <TableRow key={row.id} className="hover:bg-slate-50/30 group h-12">
              <TableCell className="w-[200px]">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 text-xs text-slate-700 bg-slate-100 px-2 py-1 rounded font-bold font-sans">
              ...
              <TableCell>
                <div className="flex flex-col">
                  <div className="font-serif font-medium text-slate-900 text-sm">
                    {row.material.product_type}
                  </div>
                  <div className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">
                    {row.material.category} {row.material.sub_category ? `// ${row.material.sub_category}` : ""}
                  </div>
                </div>
              </TableCell>
              <TableCell className="text-sm font-medium text-slate-500 font-sans">
