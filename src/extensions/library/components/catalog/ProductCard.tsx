...
import { MoreHorizontal, Trash2, ArrowRight, Image as ImageIcon } from "lucide-react";
    ...
    >
      ...
      <div className="aspect-square relative overflow-hidden bg-slate-50 border-b border-slate-100 flex items-center justify-center">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          <Badge className={cn("bg-white/95 backdrop-blur-md text-slate-900 border-none font-sans font-black px-2.5 py-1 shadow-sm uppercase tracking-[0.1em] text-[9px]", UI_ENGINE_RADIUS_ACTION)}>
            {category}
          </Badge>
          {product.catalog_status !== "APPROVED" && (
            <Badge className={cn(
              "backdrop-blur-md border-none font-sans font-black px-2.5 py-1 shadow-sm uppercase tracking-[0.1em] text-[9px]",
        ...
        <div className="absolute inset-0 bg-slate-950/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[1px]">
           <button 
             ...
             className={cn("h-12 w-12 bg-white flex items-center justify-center text-slate-900 shadow-xl scale-75 group-hover:scale-100 transition-all duration-500 hover:scale-110 active:scale-95", UI_ENGINE_RADIUS_ACTION)}
           >
      ...
      <div className="p-6">
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            <span className={cn("font-black uppercase tracking-[0.2em] text-slate-400 font-sans text-[9px]")}>
              {brand}
            </span>
            <div onClick={(e) => e.stopPropagation()}>
               <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className={cn("w-40 p-1 border-slate-100 shadow-xl font-sans bg-white text-xs", UI_ENGINE_RADIUS_CONTROL)}>
                    <DropdownMenuItem onClick={() => onDelete?.(product.id)} className={cn("py-2.5 px-3 text-red-600 focus:bg-red-50 focus:text-red-700 font-bold")}>
          ...
          <h4 className="font-serif text-base font-medium text-slate-900 leading-snug">
            {productName}
          </h4>
          
          <div className="flex items-center gap-2 mt-0.5">
             <span className="font-bold text-slate-300 font-sans tracking-wider text-[10px]">{sku}</span>
