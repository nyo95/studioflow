    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[85vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="p-6 border-b border-slate-100 bg-white">
          <DialogTitle className="font-serif text-2xl font-medium">Request Project Material</DialogTitle>
             ...
             <div className="space-y-2">
                ...
                <CreatableSearch
                  options={materials.map(m => ({ 
                    id: m.id, 
                    name: `${m.product_type} â€¢ ${m.vendor?.brand_name || "Custom"}`,
             ...
             <div className="flex flex-col md:flex-row gap-4">
               <div className="flex-1 space-y-2">
                 ...
                 <Input
                   ...
                   className="bg-slate-50 border-none h-11 focus:ring-slate-900 text-sm font-sans"
                   value={areaLocation}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2">
                  ...
                  <div className="md:col-span-2 space-y-6">
                      ...
                      <div className="space-y-2">
                          ...
                          <Input
                            ...
                            className="bg-white border-2 border-slate-100 focus:border-slate-900 h-12 text-sm font-sans transition-all"
                            value={referenceUrl}
               ...
               {materials.filter(m => m.id === selectedMaterialId).map(m => (
                 <div key={m.id} className="flex items-start gap-6 p-6 rounded-[2rem] bg-slate-50 border border-slate-100 shadow-sm transition-all hover:shadow-md">
                    <div className="h-24 w-24 rounded-2xl bg-white shadow-inner overflow-hidden flex-shrink-0 border border-slate-100">
                      {m.cover_url ? (
                        <img src={m.cover_url} alt={m.product_type} className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 space-y-2">
                       ...
                       <h3 className="font-serif text-xl font-medium text-slate-900">{m.product_type}</h3>
          ...
          {!isManualEntry && !selectedMaterialId && (
            <div className="py-20 text-center opacity-30 flex flex-col items-center gap-4 animate-in fade-in duration-500">
               ...
               <p className="text-sm font-medium font-sans">Search the archives to fetch material specs...</p>
            </div>
          )}
        </div>
