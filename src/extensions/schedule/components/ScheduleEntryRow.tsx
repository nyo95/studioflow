...
// UI Engine Schedule Tokens (Pillar 2) - design.md compliant
const UI_ENGINE_SCHEDULE_CODE_CLASS = "font-sans text-sm font-bold tracking-tight text-slate-900 uppercase";
const UI_ENGINE_SCHEDULE_NAME_CLASS = "font-sans text-sm font-medium text-slate-900 hover:underline cursor-pointer truncate leading-tight";
const UI_ENGINE_SCHEDULE_META_CLASS = "font-sans text-[10px] font-medium text-slate-400 uppercase tracking-wider";
const UI_ENGINE_SCHEDULE_BRAND_CLASS = "font-sans text-xs font-medium text-slate-500";
const UI_ENGINE_SCHEDULE_LOCATION_CLASS = "font-sans text-[11px] font-medium text-slate-600 truncate";
const UI_ENGINE_SCHEDULE_NAV_BTN_CLASS = "p-0.5 rounded hover:bg-slate-200 text-slate-400 hover:text-slate-900 transition-colors";
const UI_ENGINE_SCHEDULE_DROPDOWN_BTN_CLASS = "h-8 w-8 p-0 text-slate-400 hover:text-slate-900 hover:bg-slate-100 focus-visible:ring-0";
const UI_ENGINE_SCHEDULE_PROMOTE_BTN_CLASS = "p-1 rounded-md bg-slate-100 text-slate-400 hover:text-slate-900 hover:bg-slate-200 flex items-center justify-center shadow-sm border border-slate-200 transition-transform hover:scale-110 opacity-0 group-hover/code:opacity-100";
const UI_ENGINE_SCHEDULE_EMPTY_ADD_CLASS = "flex h-10 w-10 cursor-pointer items-center justify-center rounded-sm border border-dashed border-slate-200 bg-slate-50/50 text-[10px] font-bold text-slate-300 hover:border-slate-300 hover:text-slate-400 transition-all group/empty";
...
export function ScheduleEntryRow({ 
  ...
  const codePart = getValidString(snapshot?.specs?.product_type);
  ...
  return (
    ...
    >
      ...
      <TableCell className="w-20 px-4 py-2 align-middle relative">
        <div className="flex items-center gap-2 group/code h-full">
          ...
          {hasMultipleOptions && (
            <div className="flex flex-col items-center gap-0.5 shrink-0" onClick={e => e.stopPropagation()}>
              <button 
                ...
                className={UI_ENGINE_SCHEDULE_NAV_BTN_CLASS}
              >
          ...
          <div className="flex-1 flex items-center justify-center relative gap-2">
            <div className={UI_ENGINE_SCHEDULE_CODE_CLASS}>{entry.code}</div>
            ...
            <div className="min-w-[18px] flex items-center justify-center">
              {activeOption?.material_catalog ? (
                ...
                <button
                  ...
                  className={UI_ENGINE_SCHEDULE_PROMOTE_BTN_CLASS}
        ...
        <div className="relative inline-block group/image">
          {snapshot?.image_url ? (
            ...
            <div 
              ...
              className={UI_ENGINE_SCHEDULE_EMPTY_ADD_CLASS}
            >
      ...
      <TableCell className="px-4 py-2 align-middle flex-1">
        <div className="flex flex-col gap-0.5 truncate">
          <div 
            ...
            className={UI_ENGINE_SCHEDULE_NAME_CLASS}
          >
            {displayTitle}
          </div>
          <div className={cn(UI_ENGINE_SCHEDULE_META_CLASS, "truncate")}>
            {displayMeta}
          </div>
        </div>
      </TableCell>
      ...
      <TableCell className="w-32 px-4 py-2 align-middle">
        ...
        <div className={UI_ENGINE_SCHEDULE_LOCATION_CLASS}>{entry.location || "-"}</div>
      </TableCell>
        ...
        <div className="flex items-center justify-end opacity-0 group-hover/row:opacity-100 transition-opacity duration-200">
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
              <Button variant="ghost" className={UI_ENGINE_SCHEDULE_DROPDOWN_BTN_CLASS}>
