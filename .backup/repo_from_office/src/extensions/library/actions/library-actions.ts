  ...
  async ({ tx }) => {
    const [subCats, finishings] = await Promise.all([
      tx.materialCatalog.findMany({
        ...
        where: { sub_category: { not: null } },
        orderBy: { sub_category: "asc" },
      }),
      tx.materialCatalog.findMany({
        select: { finishing: true },
        distinct: ["finishing"],
        where: { finishing: { not: null } },
        orderBy: { finishing: "asc" },
      }),
    ]);

    return {
      subCategories: subCats.map((r) => r.sub_category as string).filter(Boolean),
      finishings: finishings.map((r) => r.finishing as string).filter(Boolean),
    };
  }
);
...
>(async ({ tx }) => {
  ...
  for (const r of rows) {
    if (r.section === "FIXTURE") fixture.push(r.category.toUpperCase());
    else material.push(r.category.toUpperCase());
  }
