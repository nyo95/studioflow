...
export const phaseService = {
  ...
  async executeApproveInternal(tx: PrismaTransaction, params: { phaseId: string; userId: string }) {
    ...
    const approvalStates: PhaseStatus[] = [PhaseStatus.ON_REVIEW_INTERNAL, PhaseStatus.ON_REVIEW_CLIENT];
    if (!approvalStates.includes(phase.status_enum)) {
      throw new ActionError(ERR.INVALID_PHASE_STATE, "INVALID_STATE");
    }
    ...
    const nextStatus = phase.status_enum === PhaseStatus.ON_REVIEW_CLIENT ? PhaseStatus.ON_REVIEW_CLIENT : PhaseStatus.APPROVED_INTERNAL;
  ...
  async executeAddActivity(
    ...
    if (revision.status_enum !== RevisionStatus.ACTIVE) {
      throw new ActionError(ERR.INVALID_PHASE_STATE, "INVALID_STATE");
    }
