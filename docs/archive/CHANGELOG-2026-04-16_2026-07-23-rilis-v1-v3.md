# Arsip Changelog — 2026-04-16 → 2026-07-23 (rilis v1.3.0 → v3.8.1)

> **Diarsipkan 2026-08-10.** Dipindahkan dari `CHANGELOG.md`.
>
> Riwayat rilis bernomor StudioFlow sejak versi pertama sampai v3.8.1. Sebagian
> besar sudah tertimbun perubahan sesudahnya — terminologi `catalog_*` sudah
> dibuang, Master Data sudah dirombak dua kali, dan bahasa UI sudah diseragamkan
> ke Inggris.
>
> Disimpan utuh karena ia satu-satunya catatan asal-usul fitur inti yang masih
> hidup (Schedule, SketchUp, MOM, RBAC, sistem revisi) — bukan sebagai acuan
> keadaan sekarang.
>
...
## [v3.8.1] - 2026-07-23 (Catalog Board Editing & Schedule Auto-Link)

### Added
- **Complete Catalog Card Editing**: Material, fixture, and schedule-only cards can now edit type, brand, item number, quantity, color/size, unit cost, notes, and image content directly from the Catalog Board.
- **Per-Card Field Visibility**: Each card can persist its own seven-field visibility override; a null override inherits the existing board-wide preference.
- **Schedule-Only Catalog Support**: Projects without a SketchUp staging record now render their Product Schedule items, and designers can add draft material or fixture entries directly from the Catalog Board.
- **Catalog Notes Contract**: Added nullable `catalog_notes` snapshot support and included notes in schedule CSV exports.

### Changed
- **Edit-Driven Schedule Bridge**: The first StudioFlow edit to an unlinked staged material or fixture now creates or resolves its Product Schedule entry automatically. Later edits mirror into that linked snapshot while explicit bulk push and merge controls remain available.
- **Lossless Push Mapping**: Item number, color/size, price, notes, quantity, unit, location, and fixture metadata now flow through single-item and bulk push paths instead of reverting to generic placeholders.
- **Staging Retention**: Full SketchUp reconciliation preserves staged rows that contain StudioFlow-managed catalog data, and FF&E sync now merges technical keys without erasing catalog metadata.

### Fixed
- **Bulk Push Entry Fields**: New material entries and fixture updates now retain staged `schedule_qty`, `schedule_unit`, and `schedule_location` values.
- **Catalog Fallback Rendering**: Removed the early empty-board result when no `SketchupProject` exists.
- **Repeatable Manual Adds**: New Catalog Board items use a reserved row only inside the creation transaction, then receive their unique normalized schedule code before commit so repeated Add actions do not collide on placeholder SKU values.

### UI Changes
- **Inline Editing Expansion**: Extended the existing click, blur, Enter, and Escape interaction to all seven catalog fields for materials, fixtures, and manual schedule items; Notes uses a wrapping multiline editor.
- **Stable Mixed-Height Grid**: Added a hover-only per-card field control while retaining the four-column, independently sized grid and existing catalog layout.
- **Direct Add Controls**: Added token-aligned “Add material” and “Add fixture” actions to the existing board toolbar, with the existing crop-and-upload image workflow shared across every item source.

## [v3.8.0] - 2026-07-21 (SketchUp Material Code Manager)

### Added
- **Material Code Manager**: Added a dedicated, valid-code-only control surface inside the SketchUp integration page. Users can group materials by prefix, merge duplicate finishes, reserve code numbers, and define a new code sequence by drag-and-drop.
- **Safe Normalization Queue**: Material number normalization now uses an ordered two-phase queue with temporary valid codes, enabling swaps and full reordering without code collisions in SketchUp.
- **Reservation Persistence**: Added `is_reserved` on staged SketchUp materials so protected material numbers are skipped during normalization.

### Fixed
- **Full Material Snapshot Reconciliation**: The SketchUp sync endpoint now removes staged material rows that no longer exist in the plugin's full coded-material snapshot, preventing merged materials from remaining in StudioFlow.
- **Deterministic Plugin Actions**: Queued merge actions now have a persisted `queue_order`; the sync API returns them in exactly that order and confirmation is idempotent.

### UI Changes
- **Integrated Workflow**: Code management appears above the existing mapping queue in `/projects/[id]/sketchup`, with clear pending-sync state and controls locked while SketchUp still has queued changes.

## [v3.7.9] — 2026-06-23 (Revision-Scoped Deliverable Upload Refresh)

### Changed
- **Deliverable Upload Workflow**: Replaced the mock deliverable dialog with a real attachment-style uploader that mirrors Project Discussion interaction patterns while uploading through `/api/upload/media`.
- **Revision-Based Temporary Semantics**: Deliverables are no longer treated like time-expiring temp attachments. A deliverable now remains active until a newer revision upload replaces it.
- **Single Active Deliverable Contract**: Uploading a new deliverable for a phase now removes the previous phase deliverable record and cleans up its managed local asset, ensuring the latest revision upload becomes the only active deliverable.

### UI Changes
- **Deliverables Table Status Hint**: Added an explicit `TEMP` badge and helper copy for outdated deliverables so users can distinguish “older revision artifact” from “missing upload”.

## [v3.7.8] — 2026-06-23 (Project MOM Extension)

### Added
- **Project MOM Extension**: Added a dedicated project-level MOM module with multiple documents per project, accessible from `/projects/[id]/mom` and linked from the `SUPERVISION` phase sidebar.
- **Structured MOM Persistence**: Added normalized MOM storage models for documents, items, points, and images, instead of reusing generic deliverable/chat entities.
- **Print-Friendly MOM Route**: Added `/projects/[id]/mom/[momId]/print` for clean print/export output without editor chrome.

### Fixed
- **Shared Image Annotation Tooling**: Extracted the screenshot/image markup editor into a reusable shared modal so Project Discussion and MOM use the same annotation behavior.

### UI Changes
- **Project Navigation**: Added `MOM Reports` as a new project extension entry in the project navigation.
- **Supervision Shortcut**: Added a dedicated CTA card in the `SUPERVISION` phase sidebar to open project MOM reports quickly.

## [v3.7.7] — 2026-06-23 (Supabase Pooling & Chat Screenshot Annotation)

### Added
- **Chat Screenshot Annotation**: Users can now copy screenshots to their clipboard and paste (`Ctrl+V`) directly in the Project Discussion textarea. This opens an inline image markup modal supporting drawing annotations (custom brush colors and sizes) with undo/clear functionality.
- **Supabase Connection Pooling Configuration**: Migrated database configuration to v7-compliant dual-connection architecture. The Next.js application runtime utilizes the Supabase transaction connection pooler (port 6543) via driver adapters, while migrations route directly through direct database connections (port 5432) using the `DIRECT_URL` fallback configuration in `prisma.config.ts`.
- **Project Discussion Sound Alert**: Added an in-browser `ding-dong` notification for newly arrived discussion messages from other users using the Web Audio API, without introducing a bundled media asset.

### Fixed
- **Prisma Client Constructor Validation**: Resolved driver adapter constructor mismatch validation error (`PrismaClientConstructorValidationError`) by restoring the `PrismaPg` adapter configured for serverless-safe execution (limiting connection pools to 1 in production).

### UI Changes
- **Markup Canvas Overlay**: Created `ScreenshotMarkupModal` styled with strict compliance to design tokens (`rounded-[var(--ui-radius-card)]`, `border-[var(--ui-border-default)]`, and standard semantic fonts). Added keyboard shortcut support (Enter to Attach, Escape to Close) and updated the button label to "Attach Screenshot (Enter)" for clearer UX guidance.
- **WhatsApp-style Unread Chat Badge**: The floating Project Discussion launcher now shows only unread incoming messages per project/user, and the badge clears automatically as soon as the discussion panel is opened/read.
